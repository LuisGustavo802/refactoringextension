...
export async function inlineFunction(code: any, selection: any, file: any) {
  const updatedCode = updateCode(code, selection);
  ...
  if (updatedCode.hasManyReturns) {
    console.log("No code changed has many returns");
    return;
  }

  if (updatedCode.isAssignedWithoutReturn) {
    console.log("No code changed isAssignedWithoutReturn");
    return;
  }

  if (updatedCode.isAssignedWithManyStatements) {
    console.log("No code changed isAssignedWithManyStatements");
    return;
  }

  if (!updatedCode.hasCodeChanged) {
    console.log("No code changed hasCodeChanged");
    return;
  }

  if (updatedCode.isExported) {
    console.log("No code changed isExported");
